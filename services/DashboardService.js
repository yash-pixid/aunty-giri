import { Activity, Screenshot, Keystroke, SystemMetric, User } from '../models/index.js';
...
class DashboardService {
   ...
   */
  async getActivityTimeline(userId, { date = new Date().toISOString().split('T')[0], interval = 'hour' } = {}) {
    try {
   ...
   */
  async getTopApps(userId, { limit = 10, startDate, endDate } = {}) {
    try {
   ...
   */
  async getWebsiteUsage(userId, { limit = 20, startDate, endDate } = {}) {
    try {
      const where = {
        userId: userId,
      ...
      websites.forEach(site => {
        try {
          ...
          domainMap[domain].total_duration += parseFloat(site.totalDuration || 0);
   ...
   */
  async getProductivityScore(userId, { startDate, endDate } = {}) {
    try {
      const where = { userId: userId };
   ...
   */
  async getScreenshots(userId, { page = 1, limit = 20, startDate, endDate } = {}) {
    try {
