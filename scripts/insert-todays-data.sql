...
-- Insert some screenshots for today
WITH user_id AS (
  SELECT id FROM users WHERE username = 'testuser' LIMIT 1
)
...
-- Insert keystroke data for today
WITH user_id AS (
  SELECT id FROM users WHERE username = 'testuser' LIMIT 1
),
apps AS (
...
FROM activities
UNION ALL
SELECT 'Screenshots', COUNT(*), MIN(created_at), MAX(created_at) FROM screenshots
UNION ALL
SELECT 'Keystrokes', COUNT(*), MIN(created_at), MAX(created_at) FROM keystrokes;
